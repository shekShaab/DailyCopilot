...
export default function OutputLibrary() {
  ...
  const loadData = async () => {
    setIsLoading(true);
    const outputsData = JSON.parse(localStorage.getItem("taskOutputs") || "[]");
    const tasksData = JSON.parse(localStorage.getItem("tasks") || "[]");

    const enrichedOutputs = outputsData.map((output) => {
      const task = tasksData.find((t) => t.id === output.task_id);
      return {
        ...output,
        agent_used: task ? task.suggested_agent : "Unknown Agent",
        created_date: task ? task.task_date : new Date().toISOString(),
      };
    });

    enrichedOutputs.sort(
      (a, b) =>
        new Date(b.created_date).getTime() -
        new Date(a.created_date).getTime()
    );

    setOutputs(enrichedOutputs);
    setTasks(tasksData);
    setIsLoading(false);
  };
  ...
  const handleSendEmail = async () => {
    alert("Email sending is not implemented in this version.");
  };
  ...
  const handleRefineOutput = async () => {
    alert("Refining output is not implemented in this version.");
  };
